...
DEFAULT_WHIP_URL = os.environ.get(
    "OPENRD_CONTROL_WHIP_URL",
    "http://43.139.25.165:8888/index/api/webrtc?app=live&stream=openrd&type=push",
)
DEFAULT_WHEP_URL = os.environ.get(
    "OPENRD_CONTROL_WHEP_URL",
    "http://43.139.25.165:8888/index/api/webrtc?app=live&stream=openrd&type=play",
)
DEFAULT_VIDEO_MODE = os.environ.get("OPENRD_CONTROL_VIDEO_MODE", "whip")
...
def transport_for_mode(mode: str) -> str:
    ...
    if normalized in {"whip", "webrtc"}:
        return "webrtc"
    if normalized in {"rtmp", "rtmp-sei"}:
        return "rtmp"
    if normalized == "rtsp":
        return "rtsp"
    return normalized
...
def snapshot_locked(vehicle: VehicleState) -> dict[str, Any]:
    ...
    mode = str(vehicle.video_status.get("mode", DEFAULT_VIDEO_MODE) or DEFAULT_VIDEO_MODE)
    payload = {
        "ok": True,
        ...
        "mode": mode,
        "transport": vehicle.video_status.get("transport") or transport_for_mode(mode),
        ...
        "whip_url": vehicle.video_status.get("whip_url") or DEFAULT_WHIP_URL,
        "whep_url": vehicle.video_status.get("whep_url") or DEFAULT_WHEP_URL,
            ...
            if action == "start":
                ...
                enqueue_command_locked(
                    ...
                    "video.start",
                    {
                        ...
                        "transport": transport_for_mode(DEFAULT_VIDEO_MODE),
